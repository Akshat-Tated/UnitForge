...
Phase 2: runs in the current Python environment (subprocess).
Phase 3 will add Docker isolation.

Usage:
    from test_runner import run_tests, TestRunResult

    result = run_tests(test_code="import pytest\\n...", module_name="calculator")
...
def _parse_pytest_passed(output: str, returncode: int | None = None) -> bool:
    ...
    Returns:
        True if the output indicates all tests passed with zero failures.
    """
    ...
    # Check for the summary line pattern like "3 passed, 1 failed"
...
def _parse_coverage_percent(output: str) -> float:
    """Extract the coverage percentage from pytest-cov or coverage output.
    ...
    """
    # coverage report lines look like: "TOTAL    100    10    90%"
    match: Optional[re.Match[str]] = re.search(r"TOTAL\s+\d+\s+\d+\s+(\d+)%", output)
    if match:
        return float(match.group(1))

    # Alternative pattern: "Coverage: 87%"
    match = re.search(r"[Cc]overage[:\s]+(\d+(?:\.\d+)?)%", output)
    if match:
        return float(match.group(1))
...
def run_tests(
    ...
    try:
        ...
        return TestRunResult(
            passed=False,
            coverage_percent=0.0,
            error_message=f"Failed to write test file: {exc}",
            output="",
            generated_file_path="",
        )
    ...
    try:
        ...
        coverage: float = _parse_coverage_percent(combined_output)
        ...
        return TestRunResult(
            ...
            generated_file_path=test_file_path,
        )
